...
# load data
tab.lu.disturb.data <- as.data.table (read.csv (here ("data/mackenzie/disturbance_by_landscape_unit_iaf.csv"),
                                               header = T,
                                               sep = ","))
# join data
sf.lu <- dplyr::full_join (sf.lu, tab.lu.disturb.data, by = "LANDSCAP_2")
...
ggplot (data = sf.awa) +
       ...
       # geom_sf_label(data = sf.caribou.herds, aes(label = CARIBOU_TA)) +
        xlab("Longitude") + ylab("Latitude") +
        ggtitle (" Base CaseDisturbance by Watershed Area in 2020") +
        scale_fill_viridis (option="inferno") +
...
# load data
tab.awa.disturb.data <- as.data.table (read.csv (here ("data/mackenzie/disturbance_by_watershed_iaf.csv"),
                                               header = T,
                                               sep = ","))
# join data
sf.awa <- dplyr::full_join (sf.awa, tab.awa.disturb.data, by = "WATERSHED_")
...
ggplot (data = crithab.plot.data.cnt, 
        ...
        scale_x_continuous (limits = c (2020, 2090), breaks = seq (2020, 2090, by = 10)) +
        scale_y_continuous (limits = c (0, 100), breaks = seq (0, 100, by = 10)) +
        ggtitle ("Indigenous Case Central Group Caribou Critical Habitat Simulated Disturbance") +
        theme (axis.text = element_text (size = 10),
               axis.title = element_text (size = 12),
               axis.line.x = element_line (size = 1),
               axis.line.y = element_line (size = 1),
